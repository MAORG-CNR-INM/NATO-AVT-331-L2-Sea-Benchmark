c----------------------------------------------------------
c      program ReGrid !former AVVIA
      subroutine regrid(igrid,ngrid,file1,file2)
c----------------------------------------------------------

      parameter (ki=1000,kj=1000)
      integer ngrid,ni,nj,nk,nzomp1,nzomp2,ipassi,inu
      integer ni2,nj2,ij1,ijn,ii1,iin,ix1,ixn,ixu1,ixun
      integer istre,jstre,segno1,segno2,iexp,jexp
      integer ni_new,nj_new,nir_new(ngrid),njr_new(ngrid)
      integer iunit,igrid

      real x(ki,kj),appox(kj),x_new(kj),
     &     y(ki,kj),appoy(kj),y_new(kj),s(kj),
     &     z(ki,kj),appoz(kj),z_new(kj),sm(kj),
     &     ys(ki),ysp(ki)
      real ftest,ntest,pesoi,pesoj,smax,tensi,tensj
      real moltiplica,divide
      real xtrasla,ytrasla,ztrasla

      real x1(ki) ,y1(ki) ,z1(ki) ,s1(ki)
      real x2(ki) ,y2(ki) ,z2(ki) ,s2(ki)
      real x2b(ki),y2b(ki),z2b(ki),s2b(ki)
      real v1(3),v2(3),v3(3)
      real ds

      character*80 file1,file2,file3
      
      namelist/REGRID_PARAMETERS/ navvia,file3,
     &                            istre,iexp,pesoi,tensi,
     &                            jstre,jexp,pesoj,tensj,
     &                            nzomp1,nzomp2,nir_new,njr_new,ipassi,
     &                            moltiplica,divide,iij,invi,invj,
     &                            imx,imy,imz,ij1,ijn,ii1,iin,
     &                            ixun,ixu1,ixn,ix1,icheck,wall,zmaxnew,
     &                            xtrasla,ytrasla,ztrasla 
...
c----------------------------------------------------------
      ...
      open(iunit,file="SBDF.nml",action="read",recl=10000)
        read(iunit,NML=REGRID_PARAMETERS)
      close(iunit)

      ni_new = nir_new(igrid)
      nj_new = njr_new(igrid)
...
c --
c        write(*,*) '------------------------------------------------'   
c        write(*,*) 'Regriding File #',iavvia
c        write(*,*) '------------------------------------------------'   

        if(nzomp1.le.1) nzomp1 = 1
        if(nzomp2.le.1) nzomp2 = 1
c
        open(1,file=file1,form='formatted',status='old')
          read(1,*) 
          read(1,*) ni,nj
          read(1,*) ((x(i,j),i=1,ni),j=1,nj),
     &              ((y(i,j),i=1,ni),j=1,nj),
     &              ((z(i,j),i=1,ni),j=1,nj)
        close(1)
...
c -- "spigoli"
        ...
        do i = 1,ni
          do j = 1,nj
            x(i,j) = x(i,j)*moltiplica/divide
            y(i,j) = y(i,j)*moltiplica/divide
            z(i,j) = z(i,j)*moltiplica/divide
          end do
        end do

        if (iij.eq.1) then
           CALL gSCAMBIO(ki,ni,nj,x,y,z)
        end if

        if (invi.eq.1) then
           CALL gINV_I(ki,ni,nj,x,y,z)
        end if

        if (invj.eq.1) then
           CALL gINV_J(ki,ni,nj,x,y,z)
        end if
...
c -- Test vari
        ...
        if (icheck.eq.1) then 
          do i=1,ni
            do j=1,nj
              if(y(i,j).lt.0) y(i,j) = 0.0
              if(abs(y(i,j)).lt.1.0e-4) y(i,j) = 0.0
            end do
          end do
        end if
        ...
        if (wall.ne.0) then
          do i=1,ni
            do j=nj-1,1,-1
              if(z(i,j).gt.wall) y(i,j) = y(i,j+1)
            end do
          end do
        end if

        if (zmaxnew.ne.0) then
          do i=1,ni
            dydz1 = (y(i,1)-y(i,2))/(z(i,1)-z(i,2))
            dydz2 = (y(i,2)-y(i,3))/(z(i,2)-z(i,3))
            dydz = 4.*dydz1/3.-dydz2/3.
            y(i,1) = y(i,1) + (zmaxnew-z(i,1))*dydz
            z(i,1) = zmaxnew
          end do
        end if
...
c -- Prima passata: fissa le coordinate sulla prima meta'
c  dei punti totali (ne prende uno si e uno no a partire
c  dal primo). Filtra lungo j

          if(nzomp1.gt.1) then
            nzomp1=nzomp1+1*segno1
            segno1= -segno1
          end if
          ...
          if(ixn.eq.1) then
            ...
            do j=1,nj
              do i=1,ni-1
                if(x(i,j).lt.x(ni,j)) x(i,j) = x(ni,j)
              end do
            end do
          end if
...
c--md per tagliare il transom dritto a 0.5
c          do j=1,nj
c            x(ni,j) = 0.02 !0.5
c          end do
c--md
          ...
          do i=1,ni
            ...
            appoy(2)     = y(i,2)
            appoz(2)     = z(i,2)
            appox(nj2-1) = x(i,nj-1)
            appoy(nj2-1) = y(i,nj-1)
            appoz(nj2-1) = z(i,nj-1)
            appox(nj2)   = x(i,nj)
            appoy(nj2)   = y(i,nj)
            appoz(nj2)   = z(i,nj)

  11        call gacurv3(appox,appoy,appoz,nj2,s)

            do k=1,nj2-1
              if(s(k).eq.s(k+1)) then
                do kk=k,nj2-1
                  s(k) = s(k+1)
                end do
                nj2 = nj2 - 1
...
c -- Rispazia le ascisse

            smax = s(nj2)

            if(smax.lt.1.0E-3) then
              ...
              if(jstre.eq.4) then
                do k=1,nj2
                  s(k) = float(k-1)/float(nj2-1)
                end do
                do k=1,nj_new
                  sm(k) = float(k-1)/float(nj_new-1)
                end do
              else
                call gstrexq(sm,nj_new,smax,jstre,pesoj,jexp)
              end if

              call ginterpolas(s,appox,nj2,sm,x_new,nj_new,tensj)
              do k=1,nj_new
                dum = x_new(k)
                call gcheckNaN0(dum)
                x(i,k) = dum
              ...
              call ginterpolas(s,appoy,nj2,sm,y_new,nj_new,tensj)
              do k=1,nj_new
                dum = y_new(k)
                call gcheckNaN0(dum)
                y(i,k) = dum
              ...
              call ginterpolas(s,appoz,nj2,sm,z_new,nj_new,tensj)
              do k=1,nj_new
                dum = z_new(k)
                call gcheckNaN0(dum)
                z(i,k) = dum
...
c
c  Seconda passata: filtra lungo i
c

          if(nzomp2.gt.1) then
            nzomp2=nzomp2+1*segno2
            segno2= -segno2
          end if
...
c -- Fissa gli estremi
            ...
            appoy(2)     = y(2,j)
            appoz(2)     = z(2,j)
            appox(ni2-1) = x(ni-1,j)
            appoy(ni2-1) = y(ni-1,j)
            appoz(ni2-1) = z(ni-1,j)
            appox(ni2)   = x(ni,j)
            appoy(ni2)   = y(ni,j)
            appoz(ni2)   = z(ni,j)

            call gacurv3(appox,appoy,appoz,ni2,s)
...
c -- Rispazia le ascisse
            ...
            if(smax.lt.1.0E-3) then
              ...
              if(istre.eq.4) then
                do k=1,ni2
                  s(k) = float(k-1)/float(ni2-1)
                end do
                do k=1,ni_new
                  sm(k) = float(k-1)/float(ni_new-1)
                end do
              else
                call gstrexq(sm,ni_new,smax,istre,pesoi,iexp)
              end if
 
              call ginterpolas(s,appox,ni2,sm,x_new,ni_new,tensi)
              ...
              call ginterpolas(s,appoy,ni2,sm,y_new,ni_new,tensi)
              do k=1,ni_new
                dum = y_new(k)
                call gcheckNaN0(dum)
                y(k,j) = dum
              ...
              call ginterpolas(s,appoz,ni2,sm,z_new,ni_new,tensi)
              do k=1,ni_new
                dum = z_new(k)
                call gcheckNaN0(dum)
                z(k,j) = dum
...
c -- Cerca di mantenere i pannelli rettangoli: da prua a poppa
c
c        do i=2,ni-1
c
c          do j=1,nj
...
c          end do
c
c          call acurv3(x1,y1,z1,nj,s1)
c          call acurv3(x2,y2,z2,nj,s2)
c          smax = s2(nj)
c          do j=1,nj
c            s2b(j) = s2(j)
c          end do
...
c            v2(3) = z2b(j )-z1(j)
c
c            if(v1(1).eq.0.and.v1(2).eq.0.and.v1(3).eq.0) go to 20
c            if(v2(1).eq.0.and.v2(2).eq.0.and.v2(3).eq.0) go to 20
c
c            call normvect(v1)
c            call normvect(v2)
c
c            call prodvect(v1,v2,v3)
c            call moduvect(v3,elle1a)
c            call scalvect(v1,v2,elle1b)
c
c            theta1a = 180*asin(elle1a)/pig
c            theta1b = 180*acos(elle1b)/pig
c
c            dth1 = abs(theta1a-theta1b)
c
c            if(dth1.gt.1.0) then
c              theta1 = 180.-theta1a
c            else
c              theta1 = (theta1a+theta1b)/2.
c            end if
c
c            if(theta1.lt.90.0) then
c
c              v2(1) = x2b(j-1)-x1(j)
c              v2(2) = y2b(j-1)-y1(j)
c              v2(3) = z2b(j-1)-z1(j)
...
c
c              call prodvect(v1,v2,v3)
c              call moduvect(v3,elle2a)
c              call scalvect(v1,v2,elle2b)
c
c              theta2a = 180*asin(elle2a)/pig
c              theta2b = 180*acos(elle2b)/pig
c
c              dth2 = abs(theta2a-theta2b)
c
c              if(dth2.gt.1.0) then
c                theta2 = 180.-theta2a
c              else
c                theta2 = (theta2a+theta2b)/2.
c              end if
c
c              ds = (s2b(j-1)-s2b(j))*(90.-theta1)/(theta2-theta1)
c              s2b(j) = s2b(j)+ds/5.
c
c              if(s2b(j).lt.0) s2b(j) = 0.0
c              if(s2b(j).gt.smax) s2b(j) = smax
c
c            else if(theta1.gt.90.0) then
c
c              v2(1) = x2b(j+1)-x1(j)
c              v2(2) = y2b(j+1)-y1(j)
c              v2(3) = z2b(j+1)-z1(j)
...
c              end if
c
c              ds = (s2b(j+1)-s2b(j))*(90.-theta1)/(theta1-theta2)
c              s2b(j) = s2b(j)+ds/5.
c
c              if(s2b(j).lt.0) s2b(j) = 0.0
c              if(s2b(j).gt.smax) s2b(j) = smax
...
c            end if
c
c            call interpola(s2,x2,nj,s2b(j),x2b(j),1)
c            call interpola(s2,y2,nj,s2b(j),y2b(j),1)
c            call interpola(s2,z2,nj,s2b(j),z2b(j),1)
c
c 20       continue
c
c          do kk=1,nj
...
c------------------------------------------------------
        ...
        nk = 1
        open(22,file=file2,form='formatted',status='unknown')
          write(22,*)  nk
          write(22,*)  ni,nj,nk
          write(22,*) ((x(i,j)+xtrasla,i=1,ni),j=1,nj),
     &                ((y(i,j)+ytrasla,i=1,ni),j=1,nj),
     &                ((z(i,j)+ztrasla,i=1,ni),j=1,nj)
        close(22)
c
        open(22,file=file3,form='formatted',status='unknown')
          write(22,*)  'VARIABLES = "X","Y","Z"'
          write(22,*)  'ZONE I=',2*nj,' J=',ni
          do i=1,ni
            do j=1,nj
              write(22,*) x(i,j)+xtrasla,y(i,j)+ytrasla,z(i,j)+ztrasla
            end do
            do j=nj,1,-1
              write(22,*) x(i,j)+xtrasla,-y(i,j)-ytrasla,z(i,j)+ztrasla
            end do
...
c-----------------------------------------------------------------------
      subroutine gSCAMBIO(ksez,nodi_i,nodi_j,x,y,z)
c-----------------------------------------------------------------------
      integer ksez,nodi_i,nodi_j,idummy
      real  x(ksez,*),y(ksez,*),z(ksez,*)
      real  xappo(1000,1000),yappo(1000,1000),zappo(1000,1000)

      write(*,*) 'Scambia i con j'
      do i = 1,nodi_i
        do j = 1,nodi_j
          xappo(j,i) = x(i,j)
          yappo(j,i) = y(i,j)
...
c-----------------------------------------------------------------------
      subroutine gINV_I(ksez,nodi_i,nodi_j,appox,appoy,appoz)
c-----------------------------------------------------------------------
      integer ksez,nodi_i,nodi_j
      real  appox(ksez,*),appoy(ksez,*),appoz(ksez,*)
      real  xappo(1000,1000),yappo(1000,400),zappo(1000,1000)

      write(*,*) 'Inverte i'
      do i = 1,nodi_i
        do j = 1,nodi_j
          xappo(nodi_i+1-i,j) = appox(i,j)
          yappo(nodi_i+1-i,j) = appoy(i,j)
          zappo(nodi_i+1-i,j) = appoz(i,j)
        end do
      end do
...
c-----------------------------------------------------------------------
      subroutine gINV_J(ksez,nodi_i,nodi_j,appox,appoy,appoz)
c-----------------------------------------------------------------------
      integer ksez,nodi_i,nodi_j
      real  appox(ksez,*),appoy(ksez,*),appoz(ksez,*)
      real  xappo(1000,1000),yappo(1000,1000),zappo(1000,1000)
      ...
      do i = 1,nodi_i
        do j = 1,nodi_j
          xappo(i,nodi_j+1-j) = appox(i,j)
          yappo(i,nodi_j+1-j) = appoy(i,j)
          zappo(i,nodi_j+1-j) = appoz(i,j)
        end do
      end do
...
c-----------------------------------------------------------------------
      subroutine gnormvect(v)
c-----------------------------------------------------------------------
      real v(3),elle
c-----------------------------------------------------------------------
      elle = v(1)**2+v(2)**2+v(3)**2
      if(elle.ne.0) elle = sqrt(elle)
      if(elle.gt.1.0e-3) then
        v(1) = v(1)/elle
        v(2) = v(2)/elle
        v(3) = v(3)/elle
      else
...
c-----------------------------------------------------------------------
      subroutine gscalvect(v1,v2,elle)
c-----------------------------------------------------------------------
      real v1(3),v2(3),elle
c-----------------------------------------------------------------------
      elle = v1(1)*v2(1)+v1(2)*v2(2)+v1(3)*v2(3)
c-----------------------------------------------------------------------
      return 
      end
c ----------------------------------------------------------------------
C
      subroutine ginterpola(x,y,np,xi,yi,ni)
...
c ----------------------------------------------------------------------
      ...
      real x(*),y(*),temp(8000),yp(8000),d1,d2,tens
      real xi(*),yi(*)
...
c ----------------------------------------------------------------------
      ...
      d1 = (y(2)-y(1))/(x(2)-x(1))
      d2 = (y(np)-y(np-1))/(x(np)-x(np-1))
      call gcurv1(np,x,y,d1,d2,ispl,yp,temp,tens,ierr)
      do i=1,ni
        yi(i) = gcurv2(xi(i),np,x,y,yp,tens)
      end do
...
c ----------------------------------------------------------------------
      ...
      subroutine ginterpolas(x,y,np,xi,yi,ni,tens)
...
c ----------------------------------------------------------------------
      ...
      integer np,ni,ierr,ispl
      real d1,d2,tens
      real xt(3000),yt(3000)
      real temp(3000),yp(3000)
      real x(*),y(*)
      real xi(*),yi(*)
...
c ----------------------------------------------------------------------
      ...
      call gcurv1(npt,xt,yt,d1,d2,ispl,yp,temp,tens,ierr)
      do i=1,ni
        yi(i) = gcurv2(xi(i),np,xt,yt,yp,tens)
      end do
...
c ----------------------------------------------------------------
      ...
      subroutine gstrex(z,npan,elle,istre,peso)
...
c  istre=2  Seno:   addensati verso lo zero
c  istre=3  Seno:   addensati verso l'estremo superiore
c-----------------------------------------------------------------
...
c-----------------------------------------------------------------
      ...
      subroutine gstrexq(z,npan,elle,istre,peso,iexpo)
...
c-----------------------------------------------------------------
      ...
      call gstrex(z,npan,elle,istre,peso)

      if(iexpo.eq.0) return
...
c-----------------------------------------------------------------
c
      subroutine gcurv1 (n,x,y,slp1,slpn,islpsw,yp,temp,
     *                  sigma,ierr)
c
...
c
    ...
    1 delx1 = x(2)-x(1)
      ...
      call gceez (delx1,delx2,sigmap,c1,c2,c3,n)
      slpp1 = c1*y(1)+c2*y(2)
    ...
    3 delxn = x(n)-x(nm1)
      ...
      call gceez (-delxn,-delxnm,sigmap,c1,c2,c3,n)
...
c
    4 delx1 = x(2)-x(1)
      ...
      dx1 = (y(2)-y(1))/delx1
      call gterms (diag1,sdiag1,sigmap,delx1)
      ...
      do 5 i = 2,nm1
        ...
        dx2 = (y(i+1)-y(i))/delx2
        call gterms (diag2,sdiag2,sigmap,delx2)
...
c
    9 ierr = 2
      ...
      subroutine gcurvs (n,x,y,d,isw,s,eps,ys,ysp,sigma,temp,
     *                  ierr)
c
      integer n,isw,ierr
...
c
      call gcurvss (n,x,y,d,isw,s,eps,ys,ysp,sigma,temp(1,1),
     *             temp(1,2),temp(1,3),temp(1,4),temp(1,5),
     *             temp(1,6),temp(1,7),temp(1,8),temp(1,9),
     *             ierr)
      return
      end
      function gcurv2 (t,n,x,y,yp,sigma)
c
      integer n
...
c
      ...
      gcurv2 = sum-del1*del2*(yp(i)*(del1+dels)+yp(im1)*
     *        (del2+dels))/(6.*dels)
      return
    1 sigdel = sigmap*dels
      call gsnhcsh (ss,dummy,sigdel,-1)
      call gsnhcsh (s1,dummy,sigmap*del1,-1)
      call gsnhcsh (s2,dummy,sigmap*del2,-1)
      gcurv2 = sum+(yp(i)*del1*(s1-ss)+yp(im1)*del2*(s2-ss))/
     *            (sigdel*sigmap*(1.+ss))
      return
      end
      function gcurvd (t,n,x,y,yp,sigma)
c
      integer n
      real t,x(n),y(n),yp(n),sigma
c
...
c
      ...
      sum = (y(i)-y(im1))/dels
      if (sigmap .ne. 0.) go to 1
      gcurvd = sum+(yp(i)*(2.*del1*del1-del2*(del1+dels))-
     *             yp(im1)*(2.*del2*del2-del1*(del2+dels)))
     *             /(6.*dels)
      return
    1 sigdel = sigmap*dels
      call gsnhcsh (ss,dummy,sigdel,-1)
      call gsnhcsh (dummy,c1,sigmap*del1,1)
      call gsnhcsh (dummy,c2,sigmap*del2,1)
      gcurvd = sum+(yp(i)*(c1-ss)-yp(im1)*(c2-ss))/
     *        (sigdel*sigmap*(1.+ss))
      return
      end
      function gcurvi (xl,xu,n,x,y,yp,sigma)
c
      integer n
...
c
      ...
      call gsnhcsh (dummy,c1,sigmap*del1,2)
      call gsnhcsh (dummy,c2,sigmap*del2,2)
      call gsnhcsh (ss,cs,sigmap*dels,3)
...
c
    8 delu1 = xxu-x(ium1)
      ...
      call gsnhcsh (dummy,cu1,sigmap*delu1,2)
      call gsnhcsh (dummy,cu2,sigmap*delu2,2)
      call gsnhcsh (dummy,cl1,sigmap*dell1,2)
      call gsnhcsh (dummy,cl2,sigmap*dell2,2)
      call gsnhcsh (ss,dummy,sigmap*dels,-1)
...
c
   10 gcurvi = ssign*sum
      ...
      subroutine gcurvp1 (n,x,y,p,yp,temp,sigma,ierr)
c
      integer n,ierr
...
c
    8 ierr = 3
      ...
      subroutine gcurvps (n,x,y,p,d,isw,s,eps,ys,ysp,sigma,
     *                   temp,ierr)
c
      integer n,isw,ierr
...
c
    ...
    2 sum = (y(i)*del1+y(im1)*del2)/dels
      ...
      gcurvp2 = sum-del1*del2*(yp(i)*(del1+dels)+yp(im1)*
     *        (del2+dels))/(6.*dels)
      return
    3 sigdel = sigmap*dels
      ...
      call gsnhcsh (s1,dummy,sigmap*del1,-1)
      call gsnhcsh (s2,dummy,sigmap*del2,-1)
      gcurvp2 = sum+(yp(i)*del1*(s1-ss)+yp(im1)*del2*(s2-ss))/
     *            (sigdel*sigmap*(1.+ss))
      return
      end
      function gcurvpi (xl,xu,n,x,y,p,yp,sigma)
c
      integer n
...
c
      x1pp = x(1)+p
      isign = 1
      ilm1 = gintrvp (xl,x,n,p,xxl)
      lper = int((xl-x(1))/p)
      if (xl .lt. x(1)) lper = lper-1
      ium1 = gintrvp (xu,x,n,p,xxu)
      uper = int((xu-x(1))/p)
...
c
   ...
   20 so = s1+s2+s6+s7
      si = s3+s4+s5+s8
      if (bdy) go to 21
      gcurvpi = float(ideltp)*(so+si)+float(isign)*si
      return
   21 gcurvpi = float(ideltp)*(so+si)+float(isign)*so
      return
      end
      subroutine gkurv1 (n,x,y,slp1,slpn,islpsw,xp,yp,temp,s,
     *                  sigma,ierr)
c
      integer n,islpsw,ierr
...
c
    ...
    2 dels1 = s(2)-s(1)
      ...
      call gceez (dels1,dels2,sigmap,c1,c2,c3,n)
      sx = c1*x(1)+c2*x(2)
      sy = c1*y(1)+c2*y(2)
      if (n .eq. 2) go to 3
      sx = sx+c3*x(3)
    ...
    5 delsn = s(n)-s(nm1)
      ...
      call gceez (-delsn,-delsnm,sigmap,c1,c2,c3,n)
      sx = c1*x(n)+c2*x(nm1)
      sy = c1*y(n)+c2*y(nm1)
      if (n .eq. 2) go to 6
      sx = sx+c3*x(n-2)
...
c
    7 dx1 = (x(2)-x(1))/s(2)
      dy1 = (y(2)-y(1))/s(2)
      call gterms (diag1,sdiag1,sigmap,s(2))
      ...
      do 8 i = 2,nm1
        ...
        dx2 = (x(i+1)-x(i))/dels2
        dy2 = (y(i+1)-y(i))/dels2
        call gterms (diag2,sdiag2,sigmap,dels2)
        diag = diag1+diag2-sdiag1*temp(i-1)
        diagin = 1./diag
...
c
   12 ierr = 2
      ...
      subroutine gkurv2 (t,xs,ys,n,x,y,xp,yp,s,sigma)
c
      integer n
...
c
    ...
    1 sigdel = sigmap*dels
      ...
      subroutine gkurvd (t,xs,ys,xst,yst,xstt,ystt,n,x,y,xp,
     *                  yp,s,sigma)
c
      integer n
...
c
    ...
    1 sigdel = sigmap*dels
      call gsnhcsh (ss,dummy,sigdel,-1)
      call gsnhcsh (s1,co1,sigmap*del1,0)
      call gsnhcsh (s2,co2,sigmap*del2,0)
      d = sigdel*sigmap*(1.+ss)
      c1 = del1*(s1-ss)/d
      c2 = del2*(s2-ss)/d
      ct1 = (co1-ss)*s(n)/d
    ...
    2 xs = sumx+c1*xp(i)+c2*xp(im1)
      ...
      subroutine gkurvp1 (n,x,y,xp,yp,temp,s,sigma,ierr)
c
      integer n,ierr
...
c
      ...
      dx1 = (x(1)-x(n))/dels1
      dy1 = (y(1)-y(n))/dels1
      call gterms(diag1,sdiag1,sigmap,dels1)
      dels2 = s(2)-s(1)
...
c
    8 ierr = 2
      ...
      subroutine gkurvp2 (t,xs,ys,n,x,y,xp,yp,s,sigma)
c
      integer n
      real t,xs,ys,x(n),y(n),xp(n),yp(n),s(n),sigma
c
...
c
      ...
      tn = s(n)*tn+s(1)
      im1 = n
      if (tn .lt. s(n)) im1 = gintrvl(tn,s,n)
      i = im1+1
      if (i .gt. n) i = 1
c
c denormalize tension factor
...
c
    ...
    1 sigdel = sigmap*dels
      ...
      subroutine gkurvpd (t,xs,ys,xst,yst,xstt,ystt,n,x,y,xp,
     *                  yp,s,sigma)
c
      integer n
      real t,xs,ys,xst,yst,xstt,ystt,x(n),y(n),xp(n),yp(n),
...
c
    ...
    2 xs = sumx+c1*xp(i)+c2*xp(im1)
      ...
      subroutine gsurf1 (m,n,x,y,z,iz,zx1,zxm,zy1,zyn,zxy11,
...
c
    ...
    2 dely1 = y(2)-y(1)
      ...
      call gceez (dely1,dely2,sigmay,c1,c2,c3,n)
      do 3 i = 1,m
...
c
    ...
    7 delyn = y(n)-y(nm1)
      ...
      call gceez (-delyn,-delynm,sigmay,c1,c2,c3,n)
      do 8 i = 1,m
        npi = n+i
...
c
   ...
   12 delx1 = x(2)-x(1)
      ...
      call gceez (delx1,delx2,sigmax,c1,c2,c3,m)
...
c
   ...
   21 delxm = x(m)-x(mm1)
      ...
      call gceez (-delxm,-delxmm,sigmax,c1,c2,c3,m)
...
c
   ...
   29   zp(i,2,1) = deli*(z(i,2)-z(i,1))
      ...
      call gterms (diag1,sdiag1,sigmay,del1)
      diagi = 1./diag1
      do 30 i = 1,m
   ...
   31     zp(i,jp1,1) = deli*(z(i,jp1)-z(i,j))
        ...
        call gterms (diag2,sdiag2,sigmay,del2)
...
c
   ...
   38   zp(2,j,3) = deli*(zp(2,j,1)-zp(1,j,1))
      call gterms (diag1,sdiag1,sigmax,del1)
      diagi = 1./diag1
      do 39 j = 1,n
   ...
   40     zp(ip1,j,3) = deli*(zp(ip1,j,1)-zp(i,j,1))
        call gterms (diag2,sdiag2,sigmax,del2)
...
c
   47 ierr = 2
      ...
      function gsurf2 (xx,yy,m,n,x,y,z,iz,zp,sigma)
c
      integer m,n,iz
...
c
     ...
     *              zp(i,j-1,3),zp(i,j,3))
      go to 2
    1 call gsnhcsh (sinhm1,dummy,sigmay*del1,-1)
      call gsnhcsh (sinhm2,dummy,sigmay*del2,-1)
      call gsnhcsh (sinhms,dummy,sigmay*dels,-1)
...
c
    2 del1 = xx-x(im1)
      ...
      gsurf2 = hermz(zim1,zi,zxxim1,zxxi)
      return
    3 call gsnhcsh (sinhm1,dummy,sigmax*del1,-1)
      call gsnhcsh (sinhm2,dummy,sigmax*del2,-1)
      call gsnhcsh (sinhms,dummy,sigmax*dels,-1)
      gsurf2 = hermnz(zim1,zi,zxxim1,zxxi,sigmax)
      return
      end
      subroutine gceez (del1,del2,sigma,c1,c2,c3,n)
c
...
c tension .ne. 0.
c
    1 call gsnhcsh (dummy,coshm1,sigma*del1,1)
      call gsnhcsh (dummy,coshm2,sigma*del2,1)
      delp = sigma*(del2+del1)/2.
      delm = sigma*(del2-del1)/2.
      call gsnhcsh (sinhmp,dummy,delp,-1)
      call gsnhcsh (sinhmm,dummy,delm,-1)
...
c
    2 c1 = -1./del1
      ...
      subroutine gcurvpp (n,x,y,p,d,isw,s,eps,ys,ysp,sigma,
...
c
      ...
      call gterms (dim1,tsd1(1),sigmap,delxi1)
      hsd1(1) = 1./delxi1
      do 1 i = 1,n
        ip1 = i+1
        if (i .eq. n) ip1 = 1
        ...
        ys(i) = delyi-delyi1
        call gterms (di,tsd1(ip1),sigmap,delxi)
        td(i) = di+dim1
...
c
   30 ierr = 6
      ...
      subroutine gcurvss (n,x,y,d,isw,s,eps,ys,ysp,sigma,td,
...
c
      ...
      do 1 i = 1,nm1
        ...
        ys(i) = delyi-delyi1
        call gterms (di,tsd1(i+1),sigmap,delxi)
        td(i) = di+dim1
        hd(i) = -(1./delxi+1./delxi1)
        hsd1(i+1) = 1./delxi
        delxi1 = delxi
...
c
    1 i = (il+ih)/2
      ...
      subroutine gsnhcsh (sinhm,coshm,x,isw)
c
      integer isw
      real sinhm,coshm,x
c
...
c
    ...
    8 expx = exp(ax)
      ...
      subroutine gterms (diag,sdiag,sigma,del)
c
...
c
      ...
      return
    1 sigdel = sigma*del
      call gsnhcsh (sinhm,coshm,sigdel,0)
