...
c-----------------------------------------------------------------------
c-----------------------------------------------------------------------
      subroutine CETRIOLO(x1,x2,x3,x4,y1,y2,y3,y4,z1,z2,z3,z4,itriqua,
     $                    npantri,eps,geofile)
      character*80 fina
      character*1  geofile
c-----------------------------------------------------------------------
c     Cerca quadrilateri con due vertici coincidenti o con
c     due lati allineati e li trasforma in triangoli
c-----------------------------------------------------------------------
      prove123 = sqrt(((y2-y1)*(z3-z2)-(z2-z1)*(y3-y2))**2 +
     $                ((z2-z1)*(x3-x2)-(x2-x1)*(z3-z2))**2 +
     $                ((x2-x1)*(y3-y2)-(y2-y1)*(x3-x2))**2)
      prove234 = sqrt(((y3-y2)*(z4-z3)-(z3-z2)*(y4-y3))**2 +
     $                ((z3-z2)*(x4-x3)-(x3-x2)*(z4-z3))**2 +
     $                ((x3-x2)*(y4-y3)-(y3-y2)*(x4-x3))**2)
      prove341 = sqrt(((y4-y3)*(z1-z4)-(z4-z3)*(y1-y4))**2 +
     $                ((z4-z3)*(x1-x4)-(x4-x3)*(z1-z4))**2 +
     $                ((x4-x3)*(y1-y4)-(y4-y3)*(x1-x4))**2)
      prove412 = sqrt(((y1-y4)*(z2-z1)-(z1-z4)*(y2-y1))**2 +
     $                ((z1-z4)*(x2-x1)-(x1-x4)*(z2-z1))**2 +
     $                ((x1-x4)*(y2-y1)-(y1-y4)*(x2-x1))**2)
      prove = min(prove123,prove234,prove341,prove412)
      if(prove.le.eps) then
c       !write(*,*)'prove=',prove
c----------- Verifica ---------
c       if (geofile.eq.'y') then
c------------------------------
c       fina(1:6) = 'quapa.'
c       write(fina(7:11),'(i4.4)') npantri
c       open(unit=99,file=fina,form='unformatted')
c       write(99) 2,2,1
c       write(99) x1,x2,x4,x3,
c    &            y1,y2,y4,y3,
c    &            z1,z2,z4,z3
c       close(99)
c       end if
c----------- Fine Verifica ----
        itriqua = 3
        if(prove.eq.prove123) then                ! si elimina punto 2 !
c         !write(*,*)'Per il pannello',npantri,'elimino punto 2'
          x2 = x3
          x3 = x4
          y2 = y3
          y3 = y4
          z2 = z3
          z3 = z4
        else if(prove.eq.prove234) then           ! si elimina punto 3 !
c         !write(*,*)'Per il pannello',npantri,'elimino punto 3'
          x3 = x4
          y3 = y4
          z3 = z4
        else if(prove.eq.prove341) then           ! si elimina punto 4 !
c         !write(*,*)'Per il pannello',npantri,'elimino punto 4'
          continue
        else if(prove.eq.prove412) then           ! si elimina punto 1 !
c         !write(*,*)'Per il pannello',npantri,'elimino punto 1'
          x1 = x4
          y1 = y4
          z1 = z4
        end if
...
c-----------------------------------------------------------------------
      subroutine CANCOSH(xn,yn,zn,xh,yh,zh,x1,x2,x3,y1,y2,y3,z1,z2,z3,
...
c-----------------------------------------------------------------------
      ...
      den = sqrt(((y2-y1)*(z3-z2)-(z2-z1)*(y3-y2))**2 +
     $           ((z2-z1)*(x3-x2)-(x2-x1)*(z3-z2))**2 +
     $           ((x2-x1)*(y3-y2)-(y2-y1)*(x3-x2))**2)
      cosxn = ((y2-y1)*(z3-z2)-(z2-z1)*(y3-y2))/den
      cosyn = ((z2-z1)*(x3-x2)-(x2-x1)*(z3-z2))/den
      coszn = ((x2-x1)*(y3-y2)-(y2-y1)*(x3-x2))/den

      a1 = x3-x2 
      b1 = y3-y2 
      c1 = z3-z2 
      d1 = x1*(x3-x2) + y1*(y3-y2) +
     $     z1*(z3-z2)
      a2 = y3-y2 
      b2 = x2-x3 
      c2 = 0.
      d2 = x2*(y3-y2) + y2*(x2-x3)
      a3 = 0.0
      b3 = z3-z2 
      c3 = y2-y3 
      d3 = y2*(z3-z2) + z2*(y2-y3)
      a4 = z3-z2
      b4 = 0.
      c4 = x2-x3
      d4 = x2*(z3-z2) + z2*(x2-x3)
     ...
     $     b4*c3*a1 - c4*a3*b1
      ...
      if(abs123.eq.dem) then
        sud = 1./d123
        xh = (d1*b2*c3 + b1*c2*d3 + c1*d2*b3 -d3*b2*c1 -
     $        b3*c2*d1 - c3*d2*b1) * sud 
        yh = (a1*d2*c3 + d1*c2*a3 + c1*a2*d3 -a3*d2*c1 -
     $        d3*c2*a1 - c3*a2*d1) * sud 
        zh = (a1*b2*d3 + b1*d2*a3 + d1*a2*b3 -a3*b2*d1 -
     $        b3*d2*a1 - d3*a2*b1) * sud
      else if(abs124.eq.dem) then
        sud = 1./d124
        xh = (d1*b2*c4 + b1*c2*d4 + c1*d2*b4 -d4*b2*c1 -
     $        b4*c2*d1 - c4*d2*b1) * sud 
        yh = (a1*d2*c4 + d1*c2*a4 + c1*a2*d4 -a4*d2*c1 -
     $        d4*c2*a1 - c4*a2*d1) * sud 
        zh = (a1*b2*d4 + b1*d2*a4 + d1*a2*b4 -a4*b2*d1 -
     $        b4*d2*a1 - d4*a2*b1) * sud
      else if(abs134.eq.dem) then
        sud = 1./d134
        xh = (d1*b3*c4 + b1*c3*d4 + c1*d3*b4 -d4*b3*c1 -
     $        b4*c3*d1 - c4*d3*b1) * sud 
        yh = (a1*d3*c4 + d1*c3*a4 + c1*a3*d4 -a4*d3*c1 -
     $        d4*c3*a1 - c4*a3*d1) * sud 
        zh = (a1*b3*d4 + b1*d3*a4 + d1*a3*b4 -a4*b3*d1 -
...
c-----------------------------------------------------------------------
      !write(*,*)'---------========= P A N C A ==========----------'
      !write(*,*)'           PANnellazione della CArena' 
      !write(*,*)' '
      !write(*,*)'      Tutti i domini devono essere numerati cosi'' '
      !write(*,*)' '
      !write(*,*)'                                          ^ z'
      !write(*,*)'                                          |'
      !write(*,*)' i <------------------------------        |'
      !write(*,*)'         |    |    |3,1 |2,1 |1,1 | x <---O  <-- U'
      !write(*,*)'         -------------------------       /'
      !write(*,*)'         |    |    |    |    |1,2 |     / '
      !write(*,*)'         -------------------------     V  y'
      !write(*,*)'         |    |    |    |    |1,3 |'
...
c-----------------------------------------------------------------------
      subroutine SCAMBIO(ksez,nodi_i,nodi_j,appox,appoy,appoz,
     &                   xappo,yappo,zappo)
c-----------------------------------------------------------------------
      dimension  appox(ksez,*),appoy(ksez,*),appoz(ksez,*),
     &           xappo(ksez,*),yappo(ksez,*),zappo(ksez,*)
...
c-----------------------------------------------------------------------
      subroutine INV_I(ksez,nodi_i,nodi_j,appox,appoy,appoz,
     &                   xappo,yappo,zappo)
c-----------------------------------------------------------------------
      dimension  appox(ksez,*),appoy(ksez,*),appoz(ksez,*),
     &           xappo(ksez,*),yappo(ksez,*),zappo(ksez,*)
...
c-----------------------------------------------------------------------
      subroutine INV_J(ksez,nodi_i,nodi_j,appox,appoy,appoz,
     &                   xappo,yappo,zappo)
c-----------------------------------------------------------------------
      dimension  appox(ksez,*),appoy(ksez,*),appoz(ksez,*),
     &           xappo(ksez,*),yappo(ksez,*),zappo(ksez,*)
...
c-----------------------------------------------------------------------
      subroutine AZZERA(ksez,klin,appox,appoy,appoz,xappo,yappo,zappo)
c-----------------------------------------------------------------------
      dimension  appox(ksez,*),appoy(ksez,*),appoz(ksez,*),
     &           xappo(ksez,*),yappo(ksez,*),zappo(ksez,*)
           do i = 1,ksez
...
c-----------------------------------------------------------------------
      subroutine COMPAREA(x1,x2,x3,x4,y1,y2,y3,y4,z1,z2,z3,z4,area)
c-----------------------------------------------------------------------
c  calcolo dell'area per eventuale compattamento
c-----------------------------------------------------------------------
      vx   = x1-x3  
      vy   = y1-y3
      vz   = z1-z3
      ...
      wy   = y4-y2
      wz   = z4-z2
      area = 0.5 * sqrt( (vy*wz-vz*wy)**2 
     &                  +(vz*wx-vx*wz)**2 
     &                  +(vx*wy-vy*wx)**2)
      return
      end
c-----------------------------------------------------------------------
      subroutine INPGRI(filename,ksez,nodi_i,nodi_j,xmirror,ymirror,
     &  zmirror,appox,appoy,appoz,ztrasla,xtrasla,dist_pp,jumbo,stre_le)
c-----------------------------------------------------------------------
      dimension  appox(ksez,*),appoy(ksez,*),appoz(ksez,*)
      character*80 filename
      character*1  xmirror,ymirror,zmirror,swath,jumbo
c

      open(unit=22,file=filename,form='formatted',status='old')
      read(22,*)
      read(22,*) nodi_i,nodi_j
      read(22,*) ((appox(i,j),i=1,nodi_i),j=1,nodi_j),
     &           ((appoy(i,j),i=1,nodi_i),j=1,nodi_j),
     &           ((appoz(i,j),i=1,nodi_i),j=1,nodi_j)
      close(22)
c-----------------------------------------------------------------------
c Se la carena viene allungata, sposta tutti i domini di DELTAX_J
c N.B. STRE_LE e' gia stato diviso per la lunghezza => 
c                                        => DELTAX_J e' gia' normalizzato
c-----------------------------------------------------------------------
      if (jumbo.eq.'u') then 
         deltax_j =+stre_le*0.5
      else if(jumbo.eq.'d') then
         deltax_j =-stre_le*0.5
      else if(jumbo.eq.'n') then
         deltax_j = 0.0
      end if 
c-----------------------------------------------------------------------
c Normalizziamo e trasliamo la carena. Anche YSWATH viene normalizzato
c-----------------------------------------------------------------------
      do j=1,nodi_j
         do i=1,nodi_i
            appox(i,j)=(appox(i,j)+xtrasla)/dist_pp + deltax_j
            appoy(i,j)=(appoy(i,j)        )/dist_pp
            appoz(i,j)=(appoz(i,j)+ztrasla)/dist_pp
         end do
      end do
c-----------------------------------------------------------------------
c Mirror rispetto ad X, Y, o Z
c-----------------------------------------------------------------------
      if(xmirror.eq.'y') then
         !write(*,*) 'Scambio x ---> -x'
         do j=1,nodi_j
            do i=1,nodi_i
               appox(i,j)= - appox(i,j)
...
C
      subroutine interpola(x,y,np,xi,yi,ni)
C________________________________________________________________
C
...
C________________________________________________________________
C
      real x(*),y(*),temp(1000),yp(1000),d1,d2,tens
      real xi(*),yi(*)
...
c ----------------------------------------------------------------------
      ...
      open(44,file='stern.profile',status='unknown')
      write(44,*) eps
      close(44)
      call system('rm -rf stern.profile')

      open(44,file='transom.grd',status='unknown')
      write(44,*) eps
      close(44)
      call system('rm -rf transom.grd')

      open(44,file='carena_0.dat',status='unknown')
      write(44,*) eps
      close(44)
      call system('rm -rf carena_0.dat')

      open(44,file='confine.grd',status='unknown')
      write(44,*) eps
      close(44)
      call system('rm -rf confine.grd')

      open(44,file='realgrid.plt',status='unknown')
      write(44,*) eps
      close(44)
      call system('rm -rf realgrid.plt')

      open(44,file='norm.dat',status='unknown')
      write(44,*) eps
      close(44)
      call system('rm -rf norm.dat')
